...
<p><b>5.	Better security</b> AOT compiles HTML templates and components into JavaScript files long before they are served to the client. With no templates to read and no risky client-side HTML or JavaScript evaluation, there are fewer opportunities for injection attacks.</p>`},
...
{
  id:46, 
  q:'Auth Gaurds, router gaurds', link:'authgaurd',
  a:`If you are trying to block some routes from loading based on some permissions or blocking a route based if not authenticated/authorised then we can use angular router guards(canActivate service)`
},
...
<p>There are two types of pipe<br/>
1. Pure 2. Impure<br/><br/>
<b>1.Pure: </b> Executed only when a pure changed to the input value<br/>
A pure change is either a change input value (string, number, boolean) or a change object referrence (Array, Object, Date)<br/>
A pure pipe is executed very fast,<br/>
By default a pipe is pure,<br/></p>
<pre>
@Pipe({
  name: 'myCustomPipe',
  pure: true   // by default pure: true if u did not pass
})
</pre>
<b>2. Impure: </b> Executed on every change even if source data does not change. (Execute unnessarily even not required)
<pre>
@Pipe({
  name: 'myCustomPipe',
  pure: false   
...

{
  id:54,
  link: 'angular',
  q:'Difference between component and directive',
  a:`<b>Component</b><br/>
  1. To register a component we used @component metadata<br/>
  2. Components are typically used to create UI Widgets<br/>
  3. Components are used to break up the application into smaller component<br/>
  4. Only one component can be present per DOM element<br/>
  5. @View decorator or templateUrl/template are mandatory<br/>
  <br/><b>Directive</b><br/>
  1. To register a Directive we used @Directive metadata<br/>
  2. Directive is used to add behavior to an existing DOM element<br/>
  3. Directive is used to design re-usable component<br/>
  4. Many Directives can be used per DOM element<br/>
  5. Directive does not use view.
  `
},
...

{
    id: 53,
    q:'Materials',
    a:`Materials offer a lot of built-in modules for your project. Features such as autocomplete, datepicker, slider, menus, grids, and toolbar are 
    available for use with materials in Angular 4.`
},
...

{
  id:52,
  q:'Module',
  a:`Module in Angular refers to a place where you can group the components, directives, pipes, and services, which are related to the application.<br/>
